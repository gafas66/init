...
First out is use-package, apparently "straight" is an alternative to this.
...
Using goldenrod as default for now, several more are available with C-c h via hydra.
...
Using doom-modeline and nerd-icons. As for all-icons above, M-x
...
Extending org mode with bullets and specific headline fonts.
...
Here are templates for capturing tasks for now
...
** Other functions like ek-set mode
