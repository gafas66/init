...
#+RESULTS:
| (lambda nil (when (string-equal major-mode eat-mode) (unless (bound-and-true-p called-once) (eat-term-send-string eat-terminal if [[ "$(hostname)" == "dsc005" ]];then ssh dsc001;fi) (eat-self-input 1 'return) (setq-local called-once t)))) | doom-modeline-invalidate-huds | doom-modeline-refresh-bars | dimmer-config-change-handler | powerline-set-selected-window | window--adjust-process-windows |
...
#+RESULTS:
: ((c . src clojure) (e . src emacs-lisp
: ) (s . src sh))
...
#+RESULTS:
: ((lambda . 955) (<= 32 (Br . Bl) 32 (Bc . Bc) 8804) (>= 32 (Br . Bl) 32 (Bc . Bc) 8805) (-> . 10230) (=> . 10233) (== . 10869) (// 32 (Br . Bl) 32 (Bc . Bc) 11005) (!= 32 (Br . Bl) 32 (Bc . Bc) 8800) (->> 32 (Br . Bl) 32 (Br . Bl) 32 (Bl . Bl) 45 (Bc . Br) 45 (Bc . Bc) 62 (Bc . Bl) 45 (Br . Br) 62))
...
#+RESULTS:
| lambda | nil | (add-to-list 'helm-completing-read-handlers-alist '(org-capture . helm-org-completing-read-tags)) | (add-to-list 'helm-completing-read-handlers-alist '(org-set-tags . helm-org-completing-read-tags)) |
...
#+RESULTS:
| lambda | nil | (define-key term-raw-map (kbd M-x) 'helm-M-x)               |
| lambda | nil | (define-key term-raw-map (kbd M-0) 'treemacs-select-window) |
| lambda | nil | (define-key term-raw-map (kbd M-o) 'ace-window)             |
